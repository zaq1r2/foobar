...
def solution(banana_list):
    possible_pairs = graph(banana_list)
    pairs = matching(possible_pairs)
...
def loop(n,m):
    normalized_n, normalized_m = normalize(n,m)
    return bool(pow(2, normalized_m, normalized_n + normalized_m))

def graph(banana_list):
    possible_pairs = defaultdict(set)
    num_monkeys = len(banana_list)
    for i in range(num_monkeys):
        for j in range(num_monkeys):
           if i != j and loop(banana_list[i], banana_list[j]):
               possible_pairs[i].add(j)
               possible_pairs[j].add(i)
...
#     pairs = max_pair_count(possible_pairs) if len(banana_list) % 2 != 0 else maximum_matching(possible_pairs)
#     if isinstance(pairs, int):
#         return len(banana_list) - pairs
#     else:
#         return len(banana_list) - len(pairs)
...
# def max_pair_count(possible_pairs):
#     l = [x for x in possible_pairs.items()]
...
# test = [1,1,1,1,1,1,1,234234324234234,1,1,4,4,4,4,4,4,4,321,1234,89234,23,234234324234234,1]
# print(solution(test))
